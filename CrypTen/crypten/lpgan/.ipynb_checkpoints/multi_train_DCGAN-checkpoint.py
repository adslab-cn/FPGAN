...
from crypten.lpgan.Secfunctions import get_network_bytes, calculate_network_traffic, print_communication_costs, print_execution_times, timeit
...
#             crypten.nn.ReLU(),
#             crypten.nn.ConvTranspose2d(ngf, nc, 4, 2, 1, bias=False),  # Adjusted kernel size, output channels, and padding
...
#             crypten.nn.Conv2d(ndf * 4, 1, 1, 1, 0, bias=False),  # kernel_size=4
...
def construct_private_model(modelD, modelG):
    ...
    if rank == 0:
        D = modelD
        D.load_state_dict(torch.load('model/modelD.pt'))
        # G = modelG
        G = Generator()
    else:
        ...
        # G = Generator()
        G = modelG
        G.load_state_dict(torch.load('model/modelG.pt'))
...
def run_lpgan():
    ...
    for epoch in range(num_epochs):
        ...
        for i, real_images in enumerate(encrypted_real_images):
            ...
            # print("d_real:", D_real.get_plain_text())
            # print("d_real_loss:", d_real_loss.get_plain_text())
    ...
    num_test_runs = 10  # 每个batchsize的运行次数
    ...
    plt.savefig('result/fake_imgs_fssgan_100.jpg', bbox_inches='tight')
