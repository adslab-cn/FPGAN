...
@contextmanager
def inference_mode():
    global ENABLE_MASURE
    ENABLE_MASURE = True
    yield
    ENABLE_MASURE = False
    
@contextmanager
def model_scope(name):
    global CURRENT_MODEL_SCOPE
    prev = CURRENT_MODEL_SCOPE
    CURRENT_MODEL_SCOPE = name
    yield 
    CURRENT_MODEL_SCOPE = prev
...
def timeit(func):
    ...
    def wrapper(*args, **kwargs):
        if ENABLE_MASURE:
            ...
            class_name = ""
            if args:
                # 实例方法：args[0] 是 self 实例
                if hasattr(args[0], "__class__") and not isinstance(args[0], type):
                    class_name = args[0].__class__.__name__ + "."
                # 类方法：args[0] 是类本身（type 类型）
                elif isinstance(args[0], type):
                    class_name = args[0].__name__ + "."

            # 生成唯一键（类名.方法名）
            key = f"{CURRENT_MODEL_SCOPE}.{class_name}{func.__name__}"
            execution_times[key] = execution_times.get(key, 0) + runtime
...
def track_network_traffic(func):
    ...
    def wrapper(*args, **kwargs):
        if ENABLE_MASURE:
            ...
            # 生成唯一键（类名.方法名）
            key = f"{CURRENT_MODEL_SCOPE}.{class_name}{func.__name__}"
            communication_costs[key] = communication_costs.get(key, 0) + traffic_diff
