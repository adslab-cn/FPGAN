...
def fss(a):
    ...
    o = o_j_1 + o_j_2
    o = o.reshape(a.shape)
    # print("o' size:", (o.reshape(a.shape)).size())
    # print("a' size:", a.size())
    # print("====funshade====", flush=True)
    return o
...
#     mask = torch.tensor([0, 0, 1]).to('cuda:0')
#     size = u.shape[-1]
...
#     x = RingTensor.convert_to_ring(y.to('cuda').int())
...
#     b0 = torch.tensor([k0[2], k0[3], k0[1]]).to('cuda:0')
#     b1 = torch.tensor([k1[2], k1[3], k1[1]]).to('cuda:0')
...
#         y0 = z1 + z2 + z3 - z4
#         # signs = torch.sign(y0)
#         # last_10_digits = torch.abs(y0) % 10**17
...
#         # signs = torch.sign(y1)
#         # last_10_digits = torch.abs(y1) % 10**17
...
def dicf(x):
    ...
    net_io_before = get_network_bytes()
    # r = random.randint(0, 100)
    down_bound = RingTensor(0)
    upper_bound = RingTensor(2**16)
    # ======================
    #     online phase
    # ======================
    # generate some values what we need to evaluate
...
def ibdicf(x):
    ...
    net_io_after = get_network_bytes()
    traffic_diff = calculate_network_traffic(net_io_before, net_io_after)
    crypten.print("traffic_ibdicf:",traffic_diff)
