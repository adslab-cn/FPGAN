...
def generate_random_ringbit_element(size, ring_size=(2**64), generator=None, **kwargs):
    """Helper function to generate a random number from a signed ring"""
    if generator is None:
        device = kwargs.get("device", torch.device("cpu"))
        device = torch.device("cpu") if device is None else device
    ...
    # 特殊处理二进制情况 (ring_size=2)
    if ring_size == 2:
        rand_element = torch.randint(
            0, 2, size, generator=generator, dtype=torch.uint8, **kwargs
        )
    else:
        rand_element = torch.randint(
            -(ring_size // 2),
...
def generate_kbit_random_tensor(size, bitlength=None, generator=None, **kwargs):
    ...
    if bitlength == 64:
        # print("===================")
        return generate_random_ring_element(size, ring_size=(2**1), generator=generator, **kwargs)
...
# @track_network_traffic
# def generate_kbit_random_tensor(size, bitlength=None, generator=None, **kwargs):
#     """Generate a random k-bit number"""
#     if bitlength is None:
#         bitlength = torch.iinfo(torch.long).bits
#     if bitlength == 64:
#         return generate_random_ringbit_element(size, ring_size=2, generator=generator, **kwargs)
#     if generator is None:
#         device = kwargs.get("device", torch.device("cpu"))
#         device = torch.device("cpu") if device is None else device
#         device = torch.device(device) if isinstance(device, str) else device
