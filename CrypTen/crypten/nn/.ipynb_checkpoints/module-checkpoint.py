...
class Module:
    ...
    def load_state_dict(self, state_dict, strict=True):
        ...
        if strict:
            ...
            print("Module._version:", Module._version)
    ...
    def update_parameters(self, learning_rate, grad_threshold=100):
        ...
        with crypten.no_grad(), torch.no_grad():
            for param in self.parameters():
                # crypten.print("param size:", param.size())
                # crypten.print("grad size:", param.grad.size())
...
class ConvTranspose(Module):
    """
    Module that performs transposed convolution (also known as deconvolution), following the ONNX specification of that
    operation. Unlike other ConvTranspose modules, this module is stateless.
    """

    def __init__(self, stride, padding, output_padding, dilation, groups=1):
        super().__init__()
        self.stride = stride
        self.padding = padding
    ...
    def forward(self, x):
        ...
        if len(x) == 2:
            ...
            raise ValueError(f"ConvTranspose module must have 2 or 3 inputs, not {len(x)}")

        # prepare inputs into transposed convolution function:
        dim = weight.dim() - 2
        if dim < 1 or dim > 2:
            raise ValueError(
                f"Transposed convolution on {dim}-dimensional input is not supported."
            )
        args = [weight]
        kwargs = {
            "stride": self.stride,
        ...
        # identify correct transposed convolution function to use:
        if torch.is_tensor(x):
            func = getattr(torch.nn.functional, f"conv_transpose{dim}d", None)
            args = [x] + args + [bias]  # torch function takes different inputs
        else:
            func = getattr(x, f"conv_transpose{dim}d", None)
    ...
    def from_onnx(attributes=None):
        ...
        if attributes is None:
            attributes = {}
        for attr in ["strides", "pads", "output_padding", "dilations"]:
            assert attr in attributes, f"missing attribute {attr}"
        ...
        output_padding = attributes["output_padding"]
        output_padding = [output_padding[idx] for idx in range(0, len(output_padding), 2)]
...
class ConvTranspose2d(Module):
    r"""
    Module that performs 2D transposed convolution.
    ...
    def __init__(
        ...
        # initialize model parameters:
        pytorch_module = torch.nn.ConvTranspose2d(
            in_channels,
            out_channels,
            kernel_size,
            stride=stride,
...
class LeakyRelu(Module):
    ...
    def forward(self, x):
        ...
        return x.fastleakyrelu(self.alpha)
        # return x * x.cmp() *(1-self.alpha) + self.alpha *  x
        # return x * x.ge(0) + self.alpha * x * x.lt(0)
        # return x.to('cpu') * x.to('cpu').fss()*(1-self.alpha) + self.alpha * x.to('cpu') 
        # return x * x.nss()*(1-self.alpha) + self.alpha * x 
        

    @staticmethod
    def from_onnx(attributes=None):
        alpha = attributes.get("alpha", 0.01)  # 从 ONNX 属性中获取 alpha
        return LeakyRelu(alpha)
    
class Tanh(Module):
    r"""Applies the Tanh function element-wise.
    ...
    Note:
        Inplace computation is not supported in the CrypTen Tanh module.
    ...
    def __init__(self, inplace=False):
        ...
        if inplace:
            logging.warning(
                "CrypTen Tanh module does not support inplace computation."
            )
...
#         # 初始化可学习参数
#         if self.affine:
#             self.weight = crypten.nn.Parameter(torch.ones(1, num_channels, 1, 1))
#             self.bias = crypten.nn.Parameter(torch.zeros(1, num_channels, 1, 1))
#         else:
...
#         # 重塑为 [B, G, C//G, H, W]
#         x_reshaped = x.view(B, self.num_groups, -1, H, W)

#         # 计算均值与方差（沿Batch、空间、组内通道维度）
#         mean = x_reshaped.mean(dim=[2, 3, 4], keepdim=True)
#         var = x_reshaped.var(dim=[2, 3, 4], keepdim=True, unbiased=False)

#         # 归一化
#         x_normalized = (x_reshaped - mean) / crypten.sqrt(var + self.eps)
