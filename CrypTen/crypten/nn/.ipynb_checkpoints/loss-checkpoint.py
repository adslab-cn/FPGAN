...
class BCELoss(_Loss):
    ...
    def forward(self, x, y):
        ...
        # print("=========BCELoss========")
        # 判断输入值的范围
        is_negative = x.min() < 0  # 判断是否包含负数
        if is_negative.get_plain_text():  # 解密后获取布尔值
            min_val = -1 + self.eps
            max_val = self.eps
        else:
            min_val = self.eps
        ...
        if self.skip_forward:
            return x.new(0)
        res = x.binary_cross_entropy(y, skip_forward=self.skip_forward)
        # print("BCELoss's input:", x.get_plain_text())
        # print("BCELoss:", res.get_plain_text())
        return res
