...
class BinarySharedTensor:
    ...
    def int_to_binary_share(self, integer, bit_length=32, device=None):
        """
        将整数转换为二进制共享。
        
        参数:
        integer (int): 要转换的整数。
        bit_length (int): 二进制表示的位长度。
        device (torch.device): 要在其上存储张量的设备。

        返回:
        BinarySharedTensor: 转换后的二进制共享。
        """
        # 将整数转换为二进制格式，并将其转换为 PyTorch 张量
        binary_str = bin(integer)[2:].zfill(bit_length)  # 将整数转换为二进制字符串并填充到指定位长度
        binary_tensor = torch.tensor([int(b) for b in binary_str], dtype=torch.uint8)
        
        # 创建 BinarySharedTensor
        tensor = BinarySharedTensor()
        tensor.share = generate_kbit_random_tensor((1, bit_length), bitlength=1, device=device) ^ binary_tensor
