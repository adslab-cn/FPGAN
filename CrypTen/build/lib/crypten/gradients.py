...
"""
将自定义的 ReLU 函数注册到 CrypTen 的 autograd 系统中
"""
import logging
import math
...
from .common.util import _grad_input_padding, _grad_input_padding_transpose
...
class AutogradReLU(AutogradFunction):
    ...
    def forward(ctx, input):
        ...
        # print("In gradients' relu's forward phase====")
        return input.mul(mask)

    @staticmethod
    def backward(ctx, grad_output):
        # print("In gradients' relu's backward phase====")
...
class AutogradMaxPool2D(AutogradFunction):
    ...
    def forward(
        ...
        # print("In gradients' max_pool2d's forward phase====")
    ...
    def backward(ctx, grad_output):
        ...
        # print("In gradients' max_pool2d's backward phase====")
...
class AutogradConv2D(AutogradFunction):
    ...
    def forward(ctx, input, kernel, stride=1, padding=0, dilation=1, groups=1):
        ...
        # crypten.print("\n=========IN CON2D'S FORWARD=======\nForward pass output size:", output.size())
        return output
    ...
    def backward(ctx, grad_output):
        ...
        # crypten.print("kernel's size:", kernel.size())
        ...
        # print("grad_kernel'size:", grad_kernel.size())
        # print(f"input of size {grad_kernel.numel()}")
        grad_kernel = grad_kernel.narrow(2, 0, kernel_size_y)
        grad_kernel = grad_kernel.narrow(3, 0, kernel_size_x)
        # print("In gradients' conv2d backward phase==========>")
        return (grad_input, grad_kernel)
...
class AutogradLeakyReLU(AutogradFunction):
    @staticmethod
    def forward(ctx, input, negative_slope=0.02):
        mask = input.gt(0.0)
        output = input.mul(mask) + negative_slope * input.mul(1-mask)
        ctx.save_for_backward(mask)
    ...
    def backward(ctx, grad_output):
        ...
        # grad_input[1-mask] *= negative_slope

        # (替换上述的索引操作）将 mask 应用于 grad_input
        masked_grad_input = grad_input * mask
        # 将负斜率应用于 (1 - mask) 的部分
        negated_masked_grad_input = grad_input * (1 - mask) * negative_slope
        # 将两部分结果相加
        grad_input = masked_grad_input + negated_masked_grad_input
...
# @register_function("conv_transpose2d")
# class AutogradConvTranspose2D(AutogradFunction):
#     @staticmethod
#     def forward(ctx, input, kernel, stride=1, padding=0, output_padding=0, groups=1, dilation=1):
#         if isinstance(stride, (int, float)):
#             stride = (stride, stride)
#         if isinstance(padding, (int, float)):
#             padding = (padding, padding)
#         if isinstance(output_padding, (int, float)):
#             output_padding = (output_padding, output_padding)
#         if isinstance(dilation, (int, float)):
#             dilation = (dilation, dilation)

#         ctx.save_multiple_for_backward(
#             (input, kernel, stride, padding, output_padding, groups, dilation)
#         )
        
#         output = input.conv_transpose2d(
#             kernel, stride=stride, padding=padding, output_padding=output_padding, groups=groups, dilation=dilation
#         )
#         # crypten.print("==========IN conv_transpose2d FORWARD========")
#         # crypten.print("Input size:", input.size())
#         # crypten.print("Kernel size:", kernel.size())
#         # crypten.print("output size:", output.size())

#         # crypten.print("Forward pass output size:", output.size())
        
#         return output

#     @staticmethod
#     def backward(ctx, grad_output):
#         input, kernel, stride, padding, output_padding, groups, dilation = ctx.saved_tensors

#         # Get input, kernel, and sizes
#         batch_size = input.size(0)
#         out_channels, in_channels, kernel_size_y, kernel_size_x = kernel.size()
#         in_channels *= groups
#         # crypten.print("==========IN conv_transpose2d BACKWARD========")
#         # crypten.print("input.size():", input.size())
#         # crypten.print("kernel.size():", kernel.size())
#         # crypten.print("grad_output.size():", grad_output.size())
#         # crypten.print("batch_size:", batch_size)
#         # crypten.print("out_channels:", out_channels)
#         # crypten.print("in_channels:", in_channels)

#         assert input.size(1) == out_channels, "wrong number of input channels"
#         assert grad_output.size(1) == in_channels, "wrong number of output channels"
#         assert grad_output.size(0) == batch_size, "wrong batch size"

#         # Compute gradient with respect to input
#         grad_input = grad_output.conv2d(
#             kernel, stride=stride, padding=padding, dilation=dilation, groups=groups
#         )
...
#             grad_output.size(3),
#         )
#         # crypten.print(f"input_size:{input.size()}")
...
#         # Compute gradient with respect to kernel
#         grad_kernel = input.conv_transpose2d(
#             grad_output,
#             stride=dilation,
#             padding=padding,
#             output_padding=output_padding,
...
class AutogradConvTranspose2D(AutogradFunction):
    ...
    def backward(ctx, grad_output):
        ...
        grad_kernel = clip_gradient(grad_kernel, max_norm=1.0)

        grad_input = normalize_gradient(grad_input)
        grad_kernel = normalize_gradient(grad_kernel)

        return grad_input, grad_kernel

def clip_gradient(grad, max_norm=1.0):
    # 手动计算 L2 范数（可以尝试加密版本，或者使用其他方法）
    grad_norm = (grad * grad).sum().sqrt()  # 手动实现 L2 范数
    if grad_norm> max_norm:
        grad.mul_(max_norm / grad_norm)  # 裁剪
    return grad


def normalize_gradient(grad, norm_type=2):
    # 手动计算 L2 范数（可以替代 torch.norm）
    if norm_type == 2:
        grad_norm = (grad * grad).sum().sqrt()  # 手动计算 L2 范数
    else:
