...
def pow(self, p, **kwargs):
    ...
    # For very large or very small exponents, we use a more stable method
    if p < -1:
        return self.reciprocal().pow(-p)  # Use the reciprocal for negative exponents
    elif p == -1:
        return self.reciprocal()  # Directly return reciprocal for -1
    elif p == 0:
        return self.new(torch.ones_like(self.data))  # 0**0 -> 1 by definition
    elif p == 1:
        return self.clone()  # x^1 = x
    elif p == 2:
        return self.square()  # x^2 is just the square
    elif p % 2 == 0:
        # For even exponents, use efficient square-and-multiply method
        return self.square().pow(p // 2)
    else:
        # For odd exponents, we use the same technique but multiply by self
        x = self.square().mul_(self)
        return x.pow((p - 1) // 2)
