...
def _update_onnx_symbolic_registry():
    ...
    if SYM_REGISTRY:
        ...
        torch.onnx.register_custom_op_symbolic(
            "aten::leaky_relu", _onnx_crypten_leaky_relu, _OPSET_VERSION
        )


@sym_help.parse_args("v", "f", "i")
def _onnx_crypten_leaky_relu(g, input, negative_slope=0.01):
    """
    This function converts PyTorch's LeakyReLU module to a LeakyReLU module in
    the ONNX model, where the negative_slope is customizable.
    """
...
#     """
#     raise DeprecationWarning(
#         "TFCrypt.nn.from_tensorflow is deprecated. ",
#         "CrypTen will no longer support model conversion from TensorFlow.",
#     )
#     # Exporting model to ONNX graph
#     if not TF_AND_TF2ONNX:
...
#     }
#     torch.onnx.export(pytorch_model, dummy_input, f, **kwargs)
#     # torch.onnx.export(pytorch_model, dummy_input, 'application/transformer.onnx', **kwargs)
#     return f


# # mapping from ONNX to TFCrypt.nn for modules with different names:
# ONNX_TO_CRYPTEN = {
#     "adaptive_avg_pool2d": module.AdaptiveAvgPool2d,
#     "adaptive_max_pool2d": module.AdaptiveMaxPool2d,
#     "AveragePool": module.AvgPool2d,
...
#         # get attributes and node type:
#         input_names = list(node.input)  #embed.weight
#         output_names = list(node.output)

#         attributes = {attr.name: _get_attribute_value(attr) for attr in node.attribute}

#         if node.op_type == 'Constant':
#             crypten_class = _get_operator_class(node.op_type)
#         else: 
#             crypten_class = _get_operator_class(node.op_type, input_names[0]) 

#         # if node.op_type == 'Slice':
#         #     attributes['starts'] = input_names[1]
#         #     attributes['ends'] = input_names[2]
#         #     attributes['axes'] = input_names[3]
#         #     attributes['steps'] = input_names[4]
#         #     print(attributes)
#         # add CrypTen module to graph:
#         crypten_module = crypten_class.from_onnx(attributes=attributes)
...
# def _get_operator_class(node_op_type, input_name=None):
#     """
#     Returns the `TFCrypt.nn.Module` type corresponding to an ONNX node.
#     """
#     crypten_class = getattr(
#         module, node_op_type, ONNX_TO_CRYPTEN.get(node_op_type, None)
#     )

#     if input_name in ['encoder.src_emb.weight', 'decoder.tgt_emb.weight', 'embed.weight']:
#         crypten_class = getattr(module, 'Embedding', None)
