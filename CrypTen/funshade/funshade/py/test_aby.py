...
l = 2         # Length (number of elements) of each vector (2**8)
...
Y_float = sample_biometric_template(K, l).flatten()
...
# import torch
# x = torch.tensor([1,2]).numpy().astype(np.int32)
# Y = torch.tensor([2,2]).numpy().astype(np.int32)
print("x:", x)
print("Y:", Y)
...
delat_y = BP.D_y
delta_x = Gate.D_x
z0 = 0*delat_y*delta_x - delta_x*d_y0 - delat_y*d_x0 + d_xy0
z1 = 1*delat_y*delta_x - delta_x*d_y1 - delat_y*d_x1 + d_xy1
delta_z0 = z0 
delta_z1 = z1
z = delta_z0 + delta_z1
print("z:", z)
...
# Exchange z_hat_j shares to reconstruct z_hat ->(aim: Reconstruction of masked input to FSS gate)
BP.z_hat_nj   = Gate.z_hat_j                 # BP: Send(z_hat_j) --> Gate
Gate.z_hat_nj = BP.z_hat_j                   # Gate: Send(z_hat_j) --> BP

print("result:", BP.z_hat_nj + BP.z_hat_j-r_in0-r_in1 )
